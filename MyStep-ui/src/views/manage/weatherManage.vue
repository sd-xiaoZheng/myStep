<template>
  <div class="weather-manage">
    ...
    <div class="operation-bar">
      ...
      >
        <template #append>
          <el-button @click="searchWeathers" :loading="searchLoading">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitWeather" :loading="submitLoading">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { getWeatherList, addWeather, updateWeather, deleteWeather } from '@/apis/api/weather'
...
export default {
  ...
  methods: {
    // 获取天气列表
    async getWeatherList(isSearch = false) {
      if (isSearch) {
    ...
    async submitWeather() {
      try {
        await this.$refs.weatherForm.validate()
        ...
        if (this.currentFile) {
          ...
          if (this.weatherForm.icon) {
            formData.append('icon', this.weatherForm.icon)
          }
        }
